...
async function carregarPsicologos() {
  ...
  if (userRole !== "ALUNO") {
    listaDiv.innerHTML =
      "<p>🛑 Acesso Negado. Apenas Alunos podem ver esta lista.</p>";
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      listaDiv.innerHTML =
        "<p>Erro ao carregar lista de psicólogos. Servidor não respondeu corretamente.</p>";
      return;
    }
    ...
    listaDiv.innerHTML =
      "<p>Não foi possível conectar ao servidor para buscar psicólogos.</p>";
  }
}
...
function filtrarPsicologos() {
  ...
  const resultados = todosPsicologos.filter((psicologo) => {
    ...
    const cpf = psicologo.cpf
      ? psicologo.cpf.replace(/\D/g, "").toLowerCase()
      : "";
...
function renderizarLista(psicologos) {
  if (psicologos.length === 0) {
    listaDiv.innerHTML =
      "<p>Nenhum psicólogo encontrado com o termo de busca.</p>";
    return;
  }

  const htmlPsicologos = psicologos
    .map((psicologo) => {
      const avatarUrl = psicologo.avatar
        ? `${URL_BASE_DA_API}/${psicologo.avatar}`
        : null;
      const styleAvatar = avatarUrl
        ? `style="background-image: url('${avatarUrl}');"`
        : "";
      const cpfExibido = psicologo.cpf
        ? psicologo.cpf.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, "$1.$2.$3-$4")
        : "N/A";

      return `
            <div class="item" data-user-id="${psicologo.id}">
                <div class="icone ${
                  psicologo.avatar ? "has-avatar" : "default"
                }" ${styleAvatar}></div>
                <span>
                ...
                <button onclick="iniciarChat(${psicologo.id}, '${
        psicologo.nome
      }')">Iniciar Chat</button>
            </div>
        `;
...
function iniciarChat(userId, userName) {
  const chatUrl = `chat?targetId=${userId}&targetName=${userName}`;
