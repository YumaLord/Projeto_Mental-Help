...
const AVATAR_PADRAO = "/img/Avatar.png";
...
async function enviarAvatar(arquivo) {
  ...
  if (isNaN(userId) || !token) {
    console.error(
      "Erro: Você precisa estar logado (ID ou Token ausente/inválido) para atualizar o avatar."
    );
    const inputAvatar = document.getElementById("input-avatar");
    if (inputAvatar) inputAvatar.value = "";
    alert(
      "Erro no envio: ID de usuário ou Token não encontrado. Tente logar novamente."
    );
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        `Falha no upload do avatar. Status: ${response.status}. Resposta: ${errorText}`
      );
    ...
    alert(
      "Erro de rede ao enviar avatar. Verifique sua conexão ou a URL da API."
    );
  } finally {
