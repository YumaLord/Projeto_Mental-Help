...
async function carregarAlunos() {
  ...
  if (!token || userRole !== "PSICOLOGO") {
    listaDiv.innerHTML =
      "<p>🛑 Acesso Negado. Faça login como Psicólogo para ver esta lista.</p>";
    return;
  }
  ...
  try {
    const response = await fetch(
      `${URL_BASE_DA_API}/usuarios/${TIPO_BUSCADO}`,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      if (response.status === 401 || response.status === 403) {
        listaDiv.innerHTML =
          "<p>Acesso Negado. Sua sessão expirou ou não possui permissão.</p>";
        return;
      }
      listaDiv.innerHTML =
        "<p>Erro ao carregar lista de alunos. Servidor não respondeu corretamente.</p>";
      return;
    }
    ...
    listaDiv.innerHTML =
      "<p>Não foi possível conectar ao servidor para buscar alunos.</p>";
  }
}
...
function renderizarLista(alunos) {
  ...
  const htmlAlunos = alunos
    .map((aluno) => {
      const avatarUrl = aluno.avatar
        ? `${URL_BASE_DA_API}/${aluno.avatar}`
        : null;
      ...
      return `
            <div class="item" data-user-id="${aluno.id}">
                <div class="icone ${
                  aluno.avatar ? "has-avatar" : "default"
                }" ${styleAvatar}></div>
                <span>
                ...
                <button onclick="iniciarChat(${aluno.id}, '${
        aluno.nome
      }')">Iniciar Chat</button>
            </div>
        `;
