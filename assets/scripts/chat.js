...
async function buscarDadosDestinatario() {
  ...
  try {
    const response = await fetch(
      `${URL_BASE_DA_API}/perfil/${destinatarioId}`,
      {
        method: "GET",
    ...
    if (response.ok) {
      ...
      console.warn(
        `[DEBUG] Falha ao carregar perfil do ID ${destinatarioId}. Status: ${response.status}. Usando nome e avatar padrão.`
      );
...
function obterParametrosURL() {
  ...
  remetenteId = sessionStorage.getItem("userId");

  if (!remetenteId || !remetenteId.trim()) {
    if (areaMensagens)
      areaMensagens.innerHTML =
        '<p class="error">Erro: Não foi possível obter o ID do usuário logado (Sessão inválida).</p>';
    return false;
  }
  if (isNaN(destinatarioId)) {
    if (areaMensagens)
      areaMensagens.innerHTML =
        '<p class="error">Erro: ID do destinatário inválido na URL.</p>';
    return false;
  }
  return true;
}
...
async function carregarHistorico() {
  if (!historicoCarregadoInicialmente) {
    areaMensagens.innerHTML =
      '<p class="loading">Carregando histórico de mensagens...</p>';
  }

  const url = `${URL_BASE_DA_API}/chat/${String(remetenteId)}/${String(
    destinatarioId
  )}`;
  const token = sessionStorage.getItem("token");
  ...
  try {
    ...
    if (!response.ok) {
      if (
        areaMensagens.children.length === 0 ||
        (areaMensagens.children.length === 1 &&
          areaMensagens.querySelector(".loading"))
      ) {
        areaMensagens.innerHTML =
          '<p class="info">Inicie uma nova conversa.</p>';
      }
      return;
    }
...
function adicionarMensagemNaTela(mensagem) {
  const tipo =
    mensagem.remetenteId === parseInt(remetenteId) ? "enviada" : "recebida";
  ...
  switch (tipoMensagem) {
    ...
    case "ARQUIVO":
      const nomeArquivo =
        mensagem.nomeArquivo || mensagem.conteudo.split("/").pop() || "arquivo";
      conteudoHTML = `<a href="${URL_BASE_DA_API}/${mensagem.conteudo}" target="_blank">Download: ${nomeArquivo}</a>`;
      break;
    case "TEXTO":
    default:
...
function rolarParaBaixo() {
  const estaPertoDoFim =
    areaMensagens.scrollHeight - areaMensagens.scrollTop <=
    areaMensagens.clientHeight + 20;
